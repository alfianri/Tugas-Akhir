...
Sebelum dilakukan tahap klasifikasi menggunkan \textit{Convolution Neural Network} (CNN) akan dilakukan 
\textit{preprocessing data}. Tujuannya untuk menghasilkan kualitas data yang baik sehingga dapat mencapai 
efisiensi yang baik dalam pelatihan model, 
untuk lebih jelaskan akan dijelaskan pada bagian selanjutnya.
...
\subsubsection{\textit{Male}}
    ...
    Kategori data \textit{male} memiliki total 163 video dalam format .avi. Deskripsi data pada fitur \textit{"action"} 
    menunjukkan empat kondisi, yaitu \textit{"talking", "yawning"}, "normal", dan {"yawning \& talking"}. Dari empat 
    kondisi ini hanya akan dipilih dengan kondisi \textit{"yawning"} dan \textit{"yawning \& talking"} hal ini bertujuan untuk 
    mendapatkan gambar atau frame yang sesuai dengan kelas yang di inginkan. Apabila kondisi lain diplih juga, hal ini
    akan memperbanyak data gambar yang tidak diperlukan, karena tidak sesuai dengan kelas yang ditetapkan. Proporsi data 
    berdasarkan fitur \textit{action} pada kategori \textit{male} dapat dilihat pada 
    Gambar \ref{Proporsi Data Male Berdasarkan "Action"} berikut.
     ...
     Dari Gambar \ref{Proporsi Data Male Berdasarkan "Action"} berdasarkan kondisi \textit{"action"}, dapat dilihat terdapat
     sebanyak 50 data untuk kondisi \textit{"normal", "talking"} berjumlah 53 data, {"yawning"} berjumlah 50 data dan
     \textit{"yawning \& talking"} berjumlah 6 data. Karena hanya kodisi \textit{"yawning"} dan \textit{"yawning \& talking"} 
     sehingga data dengan kategori \textit{male} hasil seleksi fitur berjumlah sebanyak 56 video. Semua data hasil seleksi
        fitur akan diekstrak menjadi gambar sesuai dengan kelas menjadi gambar sesuai dengan kelas yang telah
         di tentukan. Deskripsi untuk setiap data yang telah dilakukan pemilihan 
         fitur seperti \textit{participant number, glasses,  lighting} dan lain-lain pada kategori \textit{male}
          ditampilkan pada Tabel \ref{Data male setelah dilakukan pemilihan fitur} berikut.
...
\subsubsection{\textit{Female}}
    ...
    Pada kategori \textit{female} ditampilkan beberapa contoh data dengan kategori "\textit{female}" pada Gambar \ref{Female Dataset} berikut.
     ...
     Terdapat 156 video pada kategori \textit{"female"}, fitur \textit{action} pada data mencakup 
     keadaan \textit{"yawning"}, \textit{"talking"},\textit{"normal"} dan \textit{"yawning \& talking"}. 
     Berikut proprosi untuk kategori \textit{action} di tampilkan pada 
     Gambar \ref{Proporsi Data Female Berdasarkan "Action"} berikut.
...
\section{Arsitektur CNN}
    ...
    Perhitungan untuk setiap parameter pada desain arsitektur CNN ini, dijelaskan pada 
    Lampiran \ref{Perhitungan Manual Parameter CNN} berikut.
...
\subsection{Pengujian \textit{Learning Rate} dan \textit{Activation Function}}
...
\section{Analisis dan Evaluasi}
    ...
    Grafik akurasi dan \textit{loss} model terbaik ditampilkan pada Gambar \ref{Akurasi Training Model Terbaik} berikut. 
           ...
           \begin{figure}[H]
              \centering
             \includegraphics[width= 0.95\linewidth]{figures/bab4/akurasi_plotfix.png}
          ...
          \begin{figure}[H]
              \centering
              \includegraphics[width=0.95\linewidth]{figures/bab4/loss_plotfix.png}
  ...
  \subsection{Evaluasi Hasil}
    ...
    \begin{table}[H]
        \centering
        \caption[Penelitian Deteksi Kantuk Sebelumnya]{Penelitian Deteksi Kantuk Sebelumnya \cite{majeed2023detection}}
